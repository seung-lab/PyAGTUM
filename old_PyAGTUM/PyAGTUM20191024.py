    ...
    def datacollector(self):
        ...
        if self.valuelog[-1]==1 and self.valuelog[-2]==0: #cutting phase starts
            ...
            if self.downStateStart.__len__()>0:
                ...
                RetractPhaseDiff=np.average(self.parent.LEICAretractdurationlog.valuelog[-2:])-\
                    np.average(self.parent.ATUMinterslotdurationlog.valuelog[-2:])
                self.parent.RetractPhaseDifflog.datacollector(RetractPhaseDiff)
                if self.parent.cbx_synRetractSpeed.isChecked():
                    durationRatio=\
                        (np.average(self.parent.LEICAretractdurationlog.valuelog[-2:])+\
                        self.parent.sbx_targetretractphase.value())/\
                        np.average(self.parent.ATUMinterslotdurationlog.valuelog[-2:])
                    currentRetractSpeed=np.float(self.parent.retractspeedlog.valuelog[-1])
                    newRetractSpeed=int(max(10,min(5000,currentRetractSpeed+\
        ...
        if self.valuelog[-2]==1 and self.valuelog[-1]==0: #retraction phase starts
            ...
            if self.parent.cbx_synCutting.isChecked():
                CycleTime=np.average(self.parent.ATUMcyledurationlog.valuelog[-2:])
                ...
                newTapeSpeed=max(0.0,min(10.0,currentTapeSpeed+\
    ...
    def datacollector(self):
        ...
        if self.valuelog[-1]==1 and self.valuelog[-2]==0: #slot phase starts
            ...
            if self.upStateStart.__len__()>1:
                ...
                self.parent.ATUMcyledurationlog.datacollector(ATUMcycle)
...
#        print("captured frame")
    ...
    def SetPhase(self):
        phase=self.RetractPhaseDifflog.valuelog[-1]
        print("set phase to {0}".format(phase))
        self.sbx_targetretractphase.setValue(phase)
