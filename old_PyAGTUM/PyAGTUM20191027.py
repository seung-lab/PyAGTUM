            ...
            if self.parent.ATUMcyledurationlog.valuelog.__len__()>0:
                ...
                if self.parent.tapespeedlog.valuelog.__len__()>0:
                    ...
                    if self.upStateStart.__len__()>1 and self.parent.ATUMchopperlog.upStateStart.__len__()>0:
                        ...
                        dp=offset-self.parent.sbx_targetretractphase.value()
                        factor=-dp/(CycleTime+dp)             
                        newTapeSpeed=max(0.0,min(5.0,currentTapeSpeed+\
                           np.sign(self.parent.sbx_cycleFactor.value()*factor)*\
                           min(0.03,abs(self.parent.sbx_cycleFactor.value()*factor*currentTapeSpeed))))                    
                        print("offset:{0}, new tape speed:{1}".format(offset,newTapeSpeed))
                        if self.parent.cbx_PhaseLock.isChecked() and not (currentTapeSpeed==newTapeSpeed):                
                            Atum.sTS(newTapeSpeed)     
                    
#                    currentTapeSpeed=np.float(self.parent.tapespeedlog.valuelog[-1])
#                    if offset<self.parent.sbx_targetretractphase.value():
#                        newTapeSpeed=max(0.0,min(5.0,currentTapeSpeed+self.parent.sbx_phaseFactor.value()))
#                    else:
#                        newTapeSpeed=max(0.0,min(5.0,currentTapeSpeed-self.parent.sbx_phaseFactor.value()))                        
#                    print("offset:{0}, new tape speed:{1}".format(offset,newTapeSpeed))
#                    if self.parent.cbx_PhaseLock.isChecked() and not (currentTapeSpeed==newTapeSpeed):                
