...
#                print('Image read from ' + cam.name)
    ...
    #            print('{0} {1}'.format(npimg.shape[0],npimg.shape[1]))         
                ...
                if cam.name=='knife cam':
                    ...
                    self.parent.waterlevellog.waterlevel=np.mean(npimg[ROIYbegin:ROIYend,ROIXbegin:ROIXend])
...
class TapeTensionlog(log.valuelogger):    
    historylength=500
    def updateVis(self):
        1   
        ...
        if self.valuelog[-1]==1 and self.valuelog[-2]==0: #cutting phase starts
            ...
            if self.downStateStart.__len__()>0 and self.upStateStart.__len__()>0:
                ...
                if self.parent.LEICAretractdurationlog.valuelog.__len__()>0 and self.parent.ATUMinterslotdurationlog.valuelog.__len__()>0:
                    ...
                    #210118 _zz_
        #            if self.parent.retractspeedlog.valuelog.__len__()>0:
        #                print("current retract speed: {}".format(self.parent.retractspeedlog.valuelog[-1]))
...
#                        dp=offset-self.parent.sbx_targetphase.value()
#                        factor=-dp/(CycleTime+dp)     
#                        dv=np.sign(self.parent.sbx_phaseFactor.value()*factor)*\
#                           min(0.01,np.abs(self.parent.sbx_phaseFactor.value()*factor*currentTapeSpeed))
#                        newTapeSpeed=max(0.0,min(5.0,currentTapeSpeed+dv))                    
#                        print("offset:{0}, new tape speed:{1}, factor: {2}, dv: {3}".format(offset,newTapeSpeed,factor,dv))
#                        if self.parent.cbx_PhaseLock.isChecked() and not (currentTapeSpeed==newTapeSpeed):                
#                            Atum.sTS(newTapeSpeed)
                        ...
                        factor=1.0
                        if abs(offset-self.parent.sbx_targetphase.value())>2.0:
                            factor=1.25
                        elif abs(offset-self.parent.sbx_targetphase.value())<0.75 and abs(offset-self.parent.sbx_targetphase.value())>0.2:
                            factor=0.5
                        else:
                            factor=0.1
                            
                        if offset<self.parent.sbx_targetphase.value():
                            if not (self.state=='speedup') or (offset<=self.prev_offset):
                                self.state='speedup'
                                newTapeSpeed=max(0.0,min(5.0,currentTapeSpeed+factor*self.parent.sbx_phaseFactor.value()))
                             #   print("offset:{0}, {1}, new tape speed:{2}".format(offset,self.state,newTapeSpeed))
                                if self.parent.cbx_PhaseLock.isChecked() and not (currentTapeSpeed==newTapeSpeed):                
                                    self.parent.setTapeSpeed(newTapeSpeed)
                                else:
                                    self.state='none'
                           # else:
                              #  print("offset:{0}, {1}, tape speed:{2}".format(offset,self.state,currentTapeSpeed))
                        else:
                            if not (self.state=='slowdown') or (offset>=self.prev_offset):
                                self.state='slowdown'
                                newTapeSpeed=max(0.0,min(5.0,currentTapeSpeed-factor*self.parent.sbx_phaseFactor.value()))                        
                              #  print("offset:{0}, {1}, new tape speed:{2}".format(offset,self.state,newTapeSpeed))
                                if self.parent.cbx_PhaseLock.isChecked() and not (currentTapeSpeed==newTapeSpeed):                
                                    ...
                                    self.state='none'
                           # else:
                              #  print("offset:{0}, {1}, current tape speed:{2}".format(offset,self.state,currentTapeSpeed))
    ...
    def datacollector(self):
        ...
        if self.valuelog[-1]==1 and self.valuelog[-2]==0: #slot phase starts
            ...
            if self.upStateStart.__len__()>1:
                ...
                tt = round(Atum.gTT(),2)
                print("tape tension: {}".format(tt))
                self.parent.TapeTensionlog.datacollector(tt)
...
class mainGUI(QtWidgets.QMainWindow):
    ...
    def setTapeTension(self,value=None):
        if value is None:
            ...
            if value < 0:
                value = 0
            self.sbx_tapeTension.blockSignals(True)
            self.sbx_tapeTension.setValue(value)
            self.sbx_tapeTension.blockSignals(False)
        Atum.sTT(int(value))
    ...
    def setupATUMsync(self):
        ...
        self.TapeTensionlog = TapeTensionlog()
        self.TapeTensionlog.initiateTimer(1000,self._logpath,'TapeTension',parent=self)
    ...
    def SetupHardware(self):
        ...
        self.CamTimer.CamFrameRate=self._CamFrameRate
        ...
        if not self.cam_knife in self.CamTimer.CamFrames:
            self.CamTimer.CamFrames.append(self.cam_knife)
    ...
    def StartCams(self):
        ...
        newTapeTension=60
     #   self.setTapeTension(newTapeTension)
    ...
    def ConnectGUISlots(self):
        ...
        self.sbx_tapeTension.valueChanged.connect(self.setTapeTension)
...
if __name__ == "__main__":
    ...
    window = mainGUI(os.path.join(application_path,"PyAGTUM_mainwindow_210131.ui"))
