...
class LeicaCam(log.valuelogger):
    ...
    def updateVis(self):
        ...
        if ret:
            ...
            ROIXbegin=min([cf.begin.x(),cf.end.x()])
            ROIXend=max([cf.begin.x(),cf.end.x()])
            ROIYbegin=min([cf.begin.y(),cf.end.y()])
            ROIYend=max([cf.begin.y(),cf.end.y()])
...
class mainGUI(QtWidgets.QMainWindow):
    ...
    # ['CICAU1641091','CICAU1914024','CICAU1914041']
    _CamFrameRate=10;
    ...
    def SetupHardware(self):
        ...
        self.CamTimer.CameraSNs=self._CameraSNs[:2] # only use the first 2 cameras
        ...
        if not self.cam_knife in self.LeicaCam.CamFrames:
            self.LeicaCam.CamFrames.append(self.cam_knife)
            
        self.LeicaCam.setupCams()
        self.LeicaCam.initiateTimer(1000./self._CamFrameRate,None,None,parent=self)
        self.LeicaCam.start()
